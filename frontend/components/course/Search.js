...
const Search = () => {
    ...
    const searchForm = () => (
        <form onSubmit={searchSubmit}>
            <div className={`relative w-56 ${searched ? 'mt-2' : ''}`}>
                <input type="search" onChange={handleChange} className="block w-full p-4 pl-6 text-lg text-forest-100  rounded-lg bg-azur-100 focus:outline-none" placeholder="Search" required />
                <button type="submit" className="text-white absolute right-2.5 bottom-2.5  font-medium rounded-lg text-sm px-4 py-2">
                    <svg className="w-6 h-6 text-forest-100 hover:text-forest-200 " fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
